  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProveedorMaterial.fulfilled, (state, action) => {
        ...
        state.proveedor_material_options_selecteds = action.payload.data.map(({ id, nombre }) => ({
          value: nombre,
          label: nombre,
        }));
      })
      ...
      .addCase(updateProveedorMaterial.fulfilled, (state, action) => {
        state.status = "succeeded";
        const index = state.proveedorMaterial.findIndex(
          (proveedor) => proveedor.id === action.payload.id
        );
        if (index !== -1) {
      ...
      .addCase(deleteProveedorMaterial.fulfilled, (state, action) => {
        state.status = "succeeded";
        state.proveedorMaterial = state.proveedorMaterial.filter(
          (proveedor) => proveedor.id !== action.payload
        );
        state.proveedor_material_options_selecteds =
          state.proveedor_material_options_selecteds.filter(
            (proveedor) => proveedor.id !== action.payload
          );
      })
