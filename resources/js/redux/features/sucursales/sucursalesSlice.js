...
export const deleteSucursal = createAsyncThunk("sucursales/deleteSucursal", async (id) => {
  try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchSucursales.fulfilled, (state, action) => {
        ...
        state.sucursales_option_selects = action.payload.data.map(
          ({ id_sucursal, nombre, ubicacion, ...rest }) =>
        ...
        const filteredSucursales = action.payload.data
          .filter(({ id_sucursal }) => [3, 4, 7, 11].includes(id_sucursal))
        ...
        const otrosSucursales = action.payload.data.filter(
          ({ id_sucursal }) => ![3, 4, 7, 11].includes(id_sucursal)
        );

        if (otrosSucursales.length > 0) {
          filteredSucursales.push({
            id: "otros",
            name: "GIRAS INTERIOR DEL PAIS",
            color: "#bb8fce",
          });
        }
      ...
      .addCase(updateSucursal.fulfilled, (state, action) => {
        ...
        const index = state.sucursales.findIndex(
          (sucursal) => sucursal.id_sucursal === action.payload.data.id_sucursal
        );
        if (index !== -1) {
