...
export const eliminarSesionTerapiaNeonatos = createAsyncThunk(
    'TerapiaNeonatos/eliminarSesionTerapiaNeonatos',
    async (id_sesion, { rejectWithValue }) => {
        try {
            const response = await axios.delete(`${API}/terapia_optometria_neonatos/${id_sesion}`);
            return response.data;
        } catch (error) {
            return rejectWithValue(error.response.data);
        }
