...
export const fetchServicios = createAsyncThunk("servicios/fetchServicios", async ({ search }) => {
  try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchServiciosProximosAgenda.fulfilled, (state, action) => {
        ...
        state.serviciosProximos_options = state.serviciosProximos.map((servicios) => ({
      ...
      .addCase(updateServicios.fulfilled, (state, action) => {
        ...
        const index = state.servicios.findIndex(
          (servicio) => servicio.id === action.payload.data.id
        );
        if (index !== -1) {
