...
export const fetchMarcas = createAsyncThunk("marcas/fetchMarcas", async ({ search }) => {
  try {
...
export const updateMarcas = createAsyncThunk("marcas/updateMarcas", async ({ id, ...values }) => {
  try {
...
export const deleteMarcas = createAsyncThunk("marcas/deleteMarcas", async (id) => {
  try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMarcas.fulfilled, (state, action) => {
        ...
        state.marcas_lente_contacto = action.payload.data.filter(
          ({ id, codigo, nombre, lente_contacto }) => id && codigo && nombre && lente_contacto === 1
        );

        state.marcas_lente_normal = action.payload.data.filter(
          ({ id, codigo, nombre, lente_contacto }) => id && codigo && nombre && lente_contacto === 0
        );

        state.marcas_options_selecteds = action.payload.data
          .filter(
            ({ id, codigo, nombre, lente_contacto }) =>
              id && codigo && nombre && lente_contacto === 1
          )
        ...
        state.marcas_lente_normal_options_selecteds = action.payload.data
          .filter(
            ({ id, codigo, nombre, lente_contacto }) =>
              id && codigo && nombre && lente_contacto === 0
          )
          .map(({ id, codigo, nombre, ...rest }) => ({
            value: id,
            label: `${codigo} | ${nombre}`,
      ...
      .addCase(updateMarcas.fulfilled, (state, action) => {
        ...
        const indexLenteContacto = state.marcas_lente_contacto.findIndex(
          (marca) => marca.id === action.payload.data.id
        );
        if (indexLenteContacto !== -1) {
          state.marcas_lente_contacto[indexLenteContacto] = action.payload.data;
        }
        const indexLenteNormal = state.marcas_lente_normal.findIndex(
          (marca) => marca.id === action.payload.data.id
        );
        if (indexLenteNormal !== -1) {
          // Cambié indexLenteContacto por indexLenteNormal
      ...
      .addCase(deleteMarcas.fulfilled, (state, action) => {
        ...
        state.marcas_lente_contacto = state.marcas_lente_contacto.filter(
          (marca) => marca.id !== action.payload
        );
        state.marcas_lente_normal = state.marcas_lente_normal.filter(
          (marca) => marca.id !== action.payload
        );
      })
