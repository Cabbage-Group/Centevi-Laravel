...
import { clearSesionTerapiaPediatrica, fetchSesionTerapiaPediatrica } from '../../redux/features/terapias/VerSesionTerapiaPediatricaSlice.js';
import { fetchSucursales } from '../../redux/features/sucursales/sucursalesSlice.js';
import { useParams } from 'react-router-dom';


const VerSesionTerapiaPediatrica = () => {
  const dispatch = useDispatch();
  const { id_paciente, id_terapia, id_sesion } = useParams();
  const { sucursales } = useSelector((state) => state.sucursales);
  const { paciente, terapia, sesion } = useSelector((state) => state.verSesionTerapiaPediatrica);

  useEffect(() => {
    dispatch(fetchSesionTerapiaPediatrica({ id_paciente, id_terapia, id_sesion }));
    return () => {
      dispatch(clearSesionTerapiaPediatrica());
    };
...
export default VerSesionTerapiaPediatrica;