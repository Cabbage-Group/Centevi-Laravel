...
import { fetchSesionTerapiaPediatrica } from '../../redux/features/terapias/VerSesionTerapiaPediatricaSlice.js';
import { fetchSucursales } from '../../redux/features/sucursales/sucursalesSlice.js';
import { editarSesionTerapiaPediatrica } from '../../redux/features/terapias/EditarSesionTerapiaPediatricaSlice.js';
import { useParams, useNavigate } from 'react-router-dom';
import Swal from 'sweetalert2';


const EditarSesionTerapiaPediatrica = () => {

    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { id_paciente, id_terapia, id_sesion } = useParams();
    const { sucursales } = useSelector((state) => state.sucursales);
    const { paciente, terapia, status } = useSelector((state) => state.verSesionTerapiaPediatrica);
    const { status: editStatus, error: editError } = useSelector((state) => state.editarSesionTerapiaPediatrica);
    ...
    useEffect(() => {
        if (id_paciente && id_terapia) {
            dispatch(fetchSesionTerapiaPediatrica({ id_paciente, id_terapia, id_sesion }));
    ...
    const handleSubmit = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(editarSesionTerapiaPediatrica({
                    id_sesion,
...
export default EditarSesionTerapiaPediatrica