...
import {
  ...
  eliminarSesionTerapiaOrtoptica
...
const TerapiasOrtoptica = () => {
  ...
  }, [id, id_terapia, sesionModificada]);

  const getDataTerapia = async () => {
    const data = await dispatch(VerUnaTerapiaOrtoptica({ id_paciente: id, id_terapia }));
  ...
  const handleAgregarSesion = async (event) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: `¿Estás seguro de que quieres crear una nueva sesión?`,
        icon: 'question',
  ...
  const handleEliminarSesion = async (tipo, id_sesion) => {
    const sesionInfo = {
      'sesion': {
        title: 'Sesión',
        action: eliminarSesionTerapiaOrtoptica,
        fetchAction: SesionTerapiaOrtoptica
      },
    };
