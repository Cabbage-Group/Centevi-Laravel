...
import { fecthOrdenes, setFechaRange, setOrderId, setSearch, verCorrecionPdf, verOrdenPdf, verOrdenPdfSize } from '../../../redux/features/ordenes/ordenesSlice';
...
const NewTableOrdenes = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Text ellipsis title={`${record?.nombres?.trim()} ${record?.apellidos?.trim()}`}>
            ...
            <div className="widget-content widget-content-area br-6">
              <div style={{ width: '100%', marginBottom: '20px' }}>
                <div className="d-flex justify-content-between">
                  ...
                  <OptionsOrdenesLabo
                    ...
                    lenteContactoFilter={lenteContactoFilter}
                    statusFilter={statusFilter}
                    faseFilter={faseFilter}
                    proveedorFilter={proveedorFilter}
                    setLaboratorioFilter={setLaboratorioFilter}
                    setSucursalFilter={setSucursalFilter}
                    setLenteContactoFilter={setLenteContactoFilter}
              ...
              <Table
                ...
                expandable={{
                      ...
                      columns={[
                        {
                          ...
                          render: (text, record, index) => (
                            `${record.nro_orden_id}-C${index + 1}`
                          ),
                        },
                        { dataIndex: 'created_at', width: columnWidths.fecha, key: 'created_at' },
                        { dataIndex: 'sucursal', width: columnWidths.sucursal, key: 'sucursal' },
                        {
                          dataIndex: 'nombres',
                          width: columnWidths.paciente,
                          render: (_, record) => (
                        ...
                        { dataIndex: 'celular', width: columnWidths.celular, key: 'celular' },
                        { dataIndex: 'fase_actual', width: columnWidths.fase, key: 'fase_actual' },
                        {
                          dataIndex: 'estado',
                          width: columnWidths.status,
                          key: 'estado',
                          ...
                          render: (_, record, index) => (
                            <>
                              <Button
                                ...
                                icon={<FilePdfOutlined />}
                                onClick={() => {
                                  handleVerCorrecion(record.correccion_id, record.nro_orden_id + "-C" + (parseFloat(index) + 1))
                                }}
                                style={{
                                  marginRight: '8px',
                                  alignItems: "center",
                              ...
                              <Button
                                size="large"
                                icon={<EyeOutlined />}
                                onClick={() => navigate(`/ver-correcion-orden/${record.correccion_id}`)}
                                style={{
                                  marginRight: '8px',
                                  alignItems: "center",
                                  justifyContent: "center",
                  ...
                  rowExpandable: (record) => !!record.correcciones,
                  expandedRowKeys,
