  ...
  useEffect(() => {
    ...
    if (consultagenerica) {
      setFormData({
        ...
        fecha_proxima_consulta:
          moment.utc(consultagenerica.fecha_proxima_consulta).format("YYYY-MM-DD") || "",
      });

      if (consultagenerica.servicios_proximos && consultagenerica.servicios_proximos.length > 0) {
        const serviciosProximos = consultagenerica.servicios_proximos
          .map((item) => {
      ...
      if (
        consultagenerica.servicios_realizados &&
        consultagenerica.servicios_realizados.length > 0
      ) {
        const serviciosRealizados = consultagenerica?.servicios_realizados
          ?.map((item) => {
            const servicio = item.servicio; // Suponiendo que `servicio` es una propiedad anidada
            if (servicio) {
              return {
      ...
      if (
        consultagenerica.diagnostico_historia_clinica &&
        consultagenerica.diagnostico_historia_clinica.length > 0
      ) {
        const diagnosticosRealizados = consultagenerica.diagnostico_historia_clinica
          .map((item) => {
                  ...
                  >
                    <div className="statbox widget box box-shadow" data-select2-id="6">
                      ...
                      <nav aria-label="breadcrumb" className="breadcrumb-one">
                        <ol className="breadcrumb" style={{ background: "#0096881c" }}>
                          ...
                          <li aria-current="page" className="breadcrumb-item active">
                            <b>{}</b>
                            {doctorActual === consultagenerica.doctor
                              ? " (mismo doctor)"
                              : " (doctor diferente)"}
                          </li>
                        </ol>
                      ...
                      <div className="widget-content widget-content-area">
                        ...
                        <div className="form-row mb-12">
                          ...
                          <div className="form-group col-md-3">
                            <label
                              ...
                              onClick={() => console.log(formData.fecha_atencion)}
                            >
                              Fecha de atencion
                            </label>
                            <input
                        ...
                        <div className="form-row mb-12">
                          <div className="form-group col-md-6">
                            <label htmlFor="inputAddress">Fecha de proxima cita</label>
                            <input
                              className="form-control"
                              value={consultagenerica ? formData.fecha_proxima_consulta : ""}
                              ...
                              onChange={(value, val) => {
                                if (
                                  ...
                                  setFormData((prevState) => ({
                                    ...prevState,
                                    diagnostico_historia_clinica: newDiagnosticos.map(
                                      (d) => d.value
                                    ),
                                  }));
                                }
                                ...
                                <Select
                                  ...
                                  onChange={(value, val) => {
                                    if (
                                      ...
                                      setFormData((prevState) => ({
                                        ...prevState,
                                        servicios_realizados_historias_clinicas: newServicios.map(
                                          (s) => s.value
                                        ),
                                      }));
                                    }
                                ...
                                <Select
                                  ...
                                  onChange={(value, val) => {
                                    if (
                                      ...
                                      setFormData((prevState) => ({
                                        ...prevState,
                                        servicios_proximos_historias_clinicas: newServicios.map(
                                          (s) => s.value
                                        ),
                                      }));
                                    }
