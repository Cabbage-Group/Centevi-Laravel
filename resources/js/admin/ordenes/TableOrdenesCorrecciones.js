...
import { funPermisosObtenidos, funPermisosObtenidosBoolean } from '../../utils/ValidarPermisos.js';
                    ...
                    <td>
                      <div className="btn-group">
                        <Link
                          to={
                            orden.es_correccion
                              ? `/correciones-ordenes/${orden?.id_real}`
                        ...
                        <Link
                          to={
                            orden.es_correccion
                              ? `/ver-correcion-orden/${orden.id_real}`
                              : `/ver-orden/${orden.id_orden}/${orden.nro_orden_id}/${orden.id_paciente}`
                          }
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (orden.es_correccion) {
                              handleVerCorrecion(orden.id_real, orden.nro_orden_id);
                            } else {
                        ...
                        <button
                          onClick={() => {
                            if (orden.es_correccion) {
                              handleVerContactoCorreccion(orden.id_real);
                            } else {
                              handleVerContacto(orden.id_orden)
                            }
                          }}
                          className="btn-info btnAccionesOrdenes"
                          style={{
...

                        {
                          funPermisosObtenidosBoolean(permisos, 'sidebar.recetas.ordenes.eliminarorden')
                            ...
                            <button
                              onClick={() => {
                                if (orden.es_correccion) {
                                  handleEliminarCorrecionOrden(orden.id_real)
                                } else {
                                  handleEliminarOrden(orden.id_orden)
                                }
                              }}
                              borrar_receta="185"
                              ...
                              >
                                <button
                                  ...
                                  style={{
                                    ...
                                    backgroundColor: orden?.cancelada ? '#1ABC9C' : '#E7515A',
                                    color: 'white',
                                    border: 'none',
                                    borderRadius: '4px',
                                    cursor: 'pointer'
