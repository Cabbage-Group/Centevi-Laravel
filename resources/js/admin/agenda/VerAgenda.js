...
const VerAgenda = () => {
    ...
    const { servicios, serviciosProximos, serviciosProximos_options } =
    ...
    const [currentEndDateAgenda, setCurrentEndDateAgenda] = useState(
        new Date()
    );
...
`
    ...
    const { pacientes_options_agenda } = useSelector(
        (state) => state.pacientes
    );

    const { usuarios_doctores_options_selecteds } = useSelector(
    ...
    const [rangeTimeEndDateSelected, setRangeTimeEndDateSelected] =
        useState(60);
    ...
    const handleSelectChangServicios = (value, option) => {
        ...
        setProximosServicios((prev) => {
            const indexFind = prev.findIndex(
                (proximo) => proximo.value === option.value
            );
    ...
    const handleCedulaChange = (value) => {
        const paciente = pacientes_options_agenda.find(
            (paciente) => paciente.label === value
        );
        if (paciente) {
...
const downloadExcel = async () => {
    ...
    useEffect(() => {
        ...
        for (
            let m = startMonth;
            m <= (startYear === endYear ? endMonth : 12);
            m++
        ) {
            months.push(m);
        }
    ...
    const handleDateChange = (dateInfo) => {
        const { view } = dateInfo;
        const newDateCalendar = dayjs(dateInfo.view.currentStart).format(
            "MMMM YYYY"
        );
        ...
        if (clickedEvent) {
            ...
            form.setFieldsValue({
                ...
                sucursal: clickedEvent.sucursal
                    ? {
                          value: clickedEvent.sucursal_id,
                          label: clickedEvent.sucursal,
                      }
                    : undefined,
    ...
    const setTimeEndDate = (value) => {
        if (value) {
            form.setFieldsValue({
                fechaAgendaFin: dayjs(form.getFieldValue("fechaAgenda")).add(
                    value,
                    "minutes"
                ),
            });
    ...
    useEffect(() => {
        if (serviciosProximos_options.length > 0) {
            form.setFieldsValue({
                proximosServicios: serviciosProximos_options.map(
                    (serv) => serv.value
                ),
            });
        } else {
    ...
    const handleAgendarEvent = async (values) => {
        const serviciosRealizadosSubmit = proximosServicios.map(
        ...
        if (createCedula !== null) {
                    ...
                    if (result.isConfirmed) {
                        ...
                        try {
                            ...
                            if (newPaciente && newPaciente.id_paciente) {
                                ...
                                Swal.close();
                                continueAgendarEvent(
                                    values,
                                    newPaciente.id_paciente
                                );
                            } else {
                                Swal.fire({
                                    icon: "error",
            ...
            if (result.isConfirmed) {
                const data = {
                    ...
                    fecha_hora_fin:
                        values.fechaAgendaFin.format("YYYY-MM-DD HH:mm"),
    ...
    const handleUpdateEvent = async (values) => {
        ...
        if (currentEventId) {
            try {
                await dispatch(
                    updateCita({ id_cita: currentEventId, data })
                ).unwrap();
                setIsModalOpen(false);
                        ...
                        {sucursales_with_colors?.map((category) => (
                            <Col key={category.id} xxl={24} xl={24} md={24}>
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        checked={selectedSucursales.includes(
                                            category.id
                                        )}
                                        onChange={() =>
                                            handleSucursalChange(category.id)
                                        }
                                    />
                                    <div
                                        style={{
          ...
          agenda
                ...
                <FullCalendar
                    ...
                    eventContent={(info) => {
                        ...
                        const primerNombre = paciente
                            ? paciente.trim().split(" ")[0]
                            : "";
                        const primerApellido = apellidos
                            ? apellidos.trim().split(" ")[0]
                            : "";
                            ...
                            >
                                <AutoComplete
                                    ...
                                    onDropdownVisibleChange={(open) =>
                                        open && handleCedulaSelectOpen()
                                    }
                                    notFoundContent={
                                        isLoading ? <Spin size="small" /> : null
                                    }
                                    options={pacientes_options_agenda.map(
                                        (paciente) => {
                                    ...
                                    filterOption={(inputValue, option) => {
                                        const words = inputValue
                                            ...
                                            .split(" ");
                                        const fullText =
                                            `${option?.key} ${option?.searchText}`.toLowerCase();
                        ...
                        <Col xxl={12} xl={12} md={12}>
                            ...
                            >
                                <AutoComplete
                                    ...
                                    onSearch={(text) =>
                                        debouncedSetNombre(text)
                                    }
                                    // onSearch={(text) => {
                                    //   setCreatePaciente(text)
                                    // }}
                                    onDropdownVisibleChange={(open) =>
                                        open && handlePacienteSelectOpen()
                                    }
                                    notFoundContent={
                                        isLoading ? <Spin size="small" /> : null
                                    }
                        ...
                        <Col xxl={12} xl={12} md={12}>
                            ...
                            >
                                <AutoComplete
                                    ...
                                    // }}
                                    onSearch={(val) => {
                                        const valueWithPrefix = val.startsWith(
                                            "+507"
                                        )
                                            ? val
                                            : `+507${val}`;
                                        form.setFieldsValue({
                                            celular: valueWithPrefix,
                                        });
                    ...
                    <Row gutter={[16, 16]}>
                                ...
                                <Select
                                    ...
                                    onChange={(value) => {
                                        ...
                                        const sucursalSeleccionada =
                                            sucursales_option_selects.find(
                                                (sucursal) =>
                                                    sucursal.value == value
                                            );
                                        setDireccion_sucursal(
                                            sucursalSeleccionada.ubicacion_mps
                                        );
                                    }}
                                >
                                    {sucursales_option_selects.map(
                                        (sucursal) => (
                                            <Select.Option
                                                key={sucursal.value}
                        ...
                        <Col xxl={12} xl={12} md={12}>
                            ...
                            >
                                ...
                                >
                                    {usuarios_doctores_options_selecteds.map(
                                        (doctor) => (
                                            <Select.Option
                                                key={doctor.value}
                                                value={doctor.label}
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message:
                                            "La fecha y hora de fin es requerida",
                                    },
                                ]}
                            >
                                <DatePicker
                            ...
                            esProximaCita === false && {
                                validator(_, value) {
                                    ...
                                    return Promise.reject(
                                        "Debes seleccionar Terapias o Consultas"
                                    );
                                },
                            },
                        ].filter(Boolean)}
