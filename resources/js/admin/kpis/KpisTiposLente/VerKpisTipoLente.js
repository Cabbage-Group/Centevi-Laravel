...
const VerKpisTipoLente = () => {
  ...
  const renderLinesLente = () => {
    ...
    if (activeLinesLente.includes("lente_contacto")) {
      lines.push(<Bar dataKey="lente_contacto" stackId="a" fill="#3498db" barSize={70} />);
    }
    if (activeLinesLente.includes("lente_normal")) {
      lines.push(<Bar dataKey="lente_normal" stackId="a" fill="#2c3e50" barSize={70} />);
    }
    return lines;
  };
  ...
  return (
    <ResponsiveContainer width="100%" height={400}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        <DateRangeSeparate onApply={handleDateApply} onReset={handleDateReset} isMonthPicker={true} />
      ...
      <label>Buscar por Fecha:</label>
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        <DateRangeSeparate onApply={handleDateApplyAsesores} onReset={handleDateResetAsesores} isMonthPicker={true} />
        <div style={{ display: "flex", flexDirection: "column", marginTop: '-32px' }}>
          <label>
            Filtrar por Asesor:
          </label>
          ...
          >
            {asesores_activados.map(asesor => (
              <Select.Option key={asesor.id_usuario} value={asesor.id_usuario}>
                {asesor.nombre}
              </Select.Option>
            ))}
          </Select>
      ...
      <label>Buscar por Fecha:</label>
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        <DateRangeSeparate onApply={handleDateApplyDoctores} onReset={handleDateResetDoctores} isMonthPicker={true} />
        <div style={{ display: "flex", flexDirection: "column", marginTop: '-32px' }}>
          <label>
            Filtrar por Doctor:
          </label>
